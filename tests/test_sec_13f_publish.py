...
def _touch(root:Path,names):
    ...
    for name in names:
        ...
        if name.endswith('.json'):
            p.write_text(json.dumps({'data_gate_pass':True,'accepted_at_complete_rate':1.0,'ambiguous_lineage_events':0,'deterministic_us_isin_count':1010,'non_us_isin_not_evaluable_count':317}) if name=='summary.json' else '{}')
        elif name.endswith('.parquet'):
            pq.write_table(pa.table({'id':[1,2,3],'value':[1.5,None,3.25]}),p,compression='snappy')
