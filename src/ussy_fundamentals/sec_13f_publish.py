...
def _tables_value_equal(left: pa.Table, right: pa.Table) -> bool:
    if not right.schema.equals(left.schema, check_metadata=True) or right.num_rows != left.num_rows:
        return False
...
def _zstd_parquet_bytes(path: Path) -> bytes:
    ...
    with tempfile.NamedTemporaryFile(suffix='.parquet') as tmp:
        pq.write_table(table, tmp.name, compression='zstd', compression_level=PARQUET_ZSTD_LEVEL, use_dictionary=True)
        reread = pq.read_table(tmp.name)
        if not _tables_value_equal(table, reread):
            raise RuntimeError(f'Parquet ZSTD semantic verification failed for {path}')
...
def _collect(root: Path, prefix: str, group: str) -> dict[str, dict[str, Any]]:
    ...
    for path in sorted(p for p in root.rglob('*') if p.is_file()):
        rel = path.relative_to(root).as_posix(); logical_name = f'{group}/{rel}'
        logical_size = path.stat().st_size; logical_sha = _sha256(path)
        meta: dict[str, Any] = {'local_path':path,'key':f'{prefix}/{rel}','sha256':logical_sha,'logical_sha256':logical_sha,'size_bytes':logical_size,'logical_size_bytes':logical_size,'stored_size_bytes':logical_size,'representation':'identity','content_type':_content_type(path)}
        if logical_name in PARQUET_ZSTD_PATHS:
            stored = _zstd_parquet_bytes(path)
            meta.update({'stored_body':stored,'stored_sha256':_sha256_bytes(stored),'stored_size_bytes':len(stored),'compression':'zstd','compression_level':PARQUET_ZSTD_LEVEL,'representation':'parquet-zstd','compression_ratio':len(stored)/logical_size if logical_size else 1.0,'semantic_verification':'full_schema_and_value_equality'})
            ...
            if ratio <= GZIP_MAX_RATIO:
                meta.update({'key':f'{prefix}/{rel}.gz','stored_body':stored,'stored_sha256':_sha256_bytes(stored),'stored_size_bytes':len(stored),'compression':'gzip','content_encoding':'gzip','representation':'gzip','compression_ratio':ratio})
        files[rel]=meta
    return files
...
def _apply_retention(*,client,bucket:str,pointer_key:str,retain_dates:int=DEFAULT_RETAIN_SNAPSHOT_DATES)->dict:
    ...
    dates=sorted({p[len(SNAPSHOT_ROOT):].split('/')[0] for p in groups},reverse=True); keep_dates=set(dates[:retain_dates]); deleted_objects=deleted_bytes=0; deleted_prefixes=[]
