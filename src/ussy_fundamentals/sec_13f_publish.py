...
def publish(
    *,
    ...
    history_run_id: str,
    history_commit: str,
    uncertainty_run_id: str,
    uncertainty_commit: str,
    ...
    prefix = f'institutional_sponsorship/snapshots/{snapshot_date}/run-{publisher_run_id}'

    history_summary_path = history_root / 'summary.json'
    uncertainty_summary_path = uncertainty_root / 'uncertainty_summary.json'
    if not history_summary_path.exists() or not uncertainty_summary_path.exists():
        raise FileNotFoundError('Both historical summary.json and uncertainty_summary.json are required')
    ...
    required_history = ['sponsorship_state_events.parquet', 'final_period_state.parquet', 'summary.json']
    required_uncertainty = ['uncertainty_state_events.parquet', 'current_uncertainty_state.parquet', 'uncertainty_summary.json']
    missing = [str(history_root / n) for n in required_history if not (history_root / n).exists()]
    missing += [str(uncertainty_root / n) for n in required_uncertainty if not (uncertainty_root / n).exists()]
    if missing:
        raise FileNotFoundError(f'Missing canonical 13F files: {missing}')

    artifacts: dict[str, dict[str, Any]] = {}
    for group, root in [('history', history_root), ('uncertainty', uncertainty_root)]:
        group_prefix = f'{prefix}/{group}'
        for rel, meta in _collect(root, group_prefix).items():
    ...
    manifest = {
        ...
        'history_source_commit': history_commit,
        'uncertainty_source_run_id': str(uncertainty_run_id),
        'uncertainty_source_commit': uncertainty_commit,
        ...
        'semantics': {
            ...
            'ambiguity': 'CUSIP-period NOT_EVALUABLE while I_uncertain_manager_count > 0',
        },
        'artifacts': {
            name: {k: v for k, v in meta.items() if k != 'local_path'}
    ...
    pointer = {
        ...
        'deterministic_us_isin_count': history_summary.get('deterministic_us_isin_count'),
        'non_us_isin_not_evaluable_count': history_summary.get('non_us_isin_not_evaluable_count'),
        'unclassified_amendment_filings': history_summary.get('unclassified_amendment_filings'),
...
def main() -> None:
    ...
    p.add_argument('--history-commit', required=True)
    p.add_argument('--uncertainty-run-id', required=True)
    p.add_argument('--uncertainty-commit', required=True)
    ...
    manifest_key, _, pointer = publish(
        ...
        uncertainty_root=a.uncertainty_root,
        history_run_id=a.history_run_id,
        history_commit=a.history_commit,
        uncertainty_run_id=a.uncertainty_run_id,
        uncertainty_commit=a.uncertainty_commit,
        publisher_run_id=str(a.publisher_run_id),
        publisher_commit=str(a.publisher_commit),
