...
def _apply_retention(*, client, bucket: str, pointer_key: str, retain_dates: int = DEFAULT_RETAIN_SNAPSHOT_DATES) -> dict:
    if retain_dates < 2:
        raise RuntimeError('13F retention must preserve at least two distinct snapshot dates')
    pointer = json.loads(client.get_object(Bucket=bucket, Key=pointer_key)['Body'].read())
    current = str(pointer.get('snapshot_prefix') or '').rstrip('/') + '/'
    if not current.startswith(SNAPSHOT_ROOT) or '/run-' not in current:
        raise RuntimeError('current 13F pointer has invalid snapshot prefix; retention aborted')
    refs = {v for k, v in pointer.items() if k.endswith('_key') and isinstance(v, str)}
    objects = _list_snapshot_objects(client, bucket)
    groups: dict[str, list[dict]] = {}
    for obj in objects:
        key = obj['Key']; rel = key[len(SNAPSHOT_ROOT):]
        parts = rel.split('/')
        if len(parts) < 3 or not parts[1].startswith('run-'): continue
        prefix = f'{SNAPSHOT_ROOT}{parts[0]}/{parts[1]}/'
        groups.setdefault(prefix, []).append(obj)
    dates = sorted({p[len(SNAPSHOT_ROOT):].split('/')[0] for p in groups}, reverse=True)
    keep_dates = set(dates[:retain_dates])
    deleted_objects = deleted_bytes = 0; deleted_prefixes: list[str] = []
    for prefix, obs in sorted(groups.items()):
        date = prefix[len(SNAPSHOT_ROOT):].split('/')[0]
        if date in keep_dates or prefix == current: continue
        ...
        if now_current != current or prefix == now_current or keys & now_refs:
            raise RuntimeError('13F pointer moved or candidate became protected; retention aborted')
        for key in sorted(keys): client.delete_object(Bucket=bucket, Key=key)
        if any(o['Key'].startswith(prefix) for o in _list_snapshot_objects(client, bucket)):
            raise RuntimeError(f'post-delete verification failed: {prefix}')
        deleted_prefixes.append(prefix.rstrip('/')); deleted_objects += len(obs); deleted_bytes += sum(int(o.get('Size', 0)) for o in obs)
    return {'retain_distinct_snapshot_dates': retain_dates, 'kept_dates': sorted(keep_dates), 'deleted_prefixes': deleted_prefixes, 'deleted_objects': deleted_objects, 'deleted_bytes': deleted_bytes}


def publish(*, history_root: Path, uncertainty_root: Path, live_root: Path,
            history_run_id: str, history_commit: str, uncertainty_run_id: str, uncertainty_commit: str,
            live_run_id: str, live_commit: str, publisher_run_id: str, publisher_commit: str,
            pointer_key: str = 'institutional_sponsorship/current.json', bucket: str | None = None, client=None) -> tuple[str, dict, dict]:
    client = client or r2_client(); bucket = bucket or _env('R2_BUCKET_NAME')
    now = datetime.now(timezone.utc); produced_at = now.isoformat().replace('+00:00', 'Z')
    prefix = f'{SNAPSHOT_ROOT}{now.date().isoformat()}/run-{publisher_run_id}'
    history_summary_path = history_root / 'summary.json'; uncertainty_summary_path = uncertainty_root / 'uncertainty_summary.json'; live_summary_path = live_root / 'summary.json'
    if not history_summary_path.exists() or not uncertainty_summary_path.exists() or not live_summary_path.exists(): raise FileNotFoundError('Historical, uncertainty, and live summary files are required')
    history_summary = json.loads(history_summary_path.read_text(encoding='utf-8')); uncertainty_summary = json.loads(uncertainty_summary_path.read_text(encoding='utf-8')); live_summary = json.loads(live_summary_path.read_text(encoding='utf-8'))
    ...
    pointer['retention_result'] = retention
    return manifest_key,manifest,pointer


def main() -> None:
    p=argparse.ArgumentParser(); p.add_argument('--history-root',type=Path,required=True); p.add_argument('--uncertainty-root',type=Path,required=True); p.add_argument('--live-root',type=Path,required=True)
    p.add_argument('--history-run-id',required=True); p.add_argument('--history-commit',required=True); p.add_argument('--uncertainty-run-id',required=True); p.add_argument('--uncertainty-commit',required=True); p.add_argument('--live-run-id',required=True); p.add_argument('--live-commit',required=True); p.add_argument('--publisher-run-id',default=os.getenv('GITHUB_RUN_ID')); p.add_argument('--publisher-commit',default=os.getenv('GITHUB_SHA')); p.add_argument('--pointer-key',default='institutional_sponsorship/current.json')
    a=p.parse_args()
    if not a.publisher_run_id or not a.publisher_commit: raise RuntimeError('publisher run id and commit are required')
    manifest_key,_,pointer=publish(history_root=a.history_root,uncertainty_root=a.uncertainty_root,live_root=a.live_root,history_run_id=a.history_run_id,history_commit=a.history_commit,uncertainty_run_id=a.uncertainty_run_id,uncertainty_commit=a.uncertainty_commit,live_run_id=a.live_run_id,live_commit=a.live_commit,publisher_run_id=str(a.publisher_run_id),publisher_commit=str(a.publisher_commit),pointer_key=a.pointer_key)
